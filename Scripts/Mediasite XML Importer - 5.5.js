...
var squarg: StreamReader = new StreamReader(wharr + "\\MediasitePresentation_50.xml");	// Reads an external file and executes code
var line : String;																		// for each non-empty line of it
while ((line = squarg.ReadLine()) != null) {											
		if (line != String.Empty) {
			if (line.substring(0, 14) == "        <Time>") {							// If a timestamp is detected in the XML file
				yarr.Add(line.slice(14, -7));											// Store the timestamp in the array
			}
		}
	}
squarg.Close();
